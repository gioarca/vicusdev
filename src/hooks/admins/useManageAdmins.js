...
export const useManageAdmin = () => {
  ...
  const updateAdmin = async ({ formData }) => {
    ...
    // Determiniamo se è un aggiornamento profilo standard o un cambio password
    const isPasswordChange = formData.newPassword && formData.currentPassword;

    // Prepariamo i dati da inviare al server
    const requestData = { ...formData };

    // Se abbiamo una richiesta di cambio password
    if (isPasswordChange) {
      // Inviamo i dati necessari per il cambio password:
      // 1. La password attuale per verificare l'identità
      // 2. La nuova password da impostare
      requestData.password = formData.currentPassword;
      requestData.newPassword = formData.newPassword;
    } else {
      // Per aggiornamenti normali del profilo, la password corrente
      // è richiesta per l'autenticazione
      requestData.password = formData.currentPassword || formData.password;
    }
    ...
    if (response?.status === 200) {
      ...
      // Messaggio di successo diverso in base all'operazione
      if (isPasswordChange) {
  ...
  const changePassword = async ({ currentPassword, newPassword }) => {
    ...
    if (!currentPassword || !newPassword) {
      toast.error("Password attuale e nuova password sono obbligatorie");
      return false;
    }
  ...
  return {
    ...
    createAdmin,
    updateAdmin,
    changePassword, // Aggiungiamo la nuova funzione per il cambio password
    deleteAdmin,
  };
};
